...
const projects = [
  {
    ...
    text: "Mobile-first online pharmacy platform using React & Vanilla JS. Features localStorage cart, digital prescription upload, and dynamic filters for 500+ medical SKUs. Improved page load by 25%. Live Demo:👉 https://vyaish.vercel.app/"
...
const TagCard = ({ number, title, text, tags, link, period, className, aosDelay, aosType, pathLength, containerRef }) => {
  ...
  return (
    ...
    >
      ...
      <div className={`w-full h-full rounded-[1.5rem] mt-8 p-6 flex flex-col min-h-[260px] transition-colors duration-700 ${isActive ? 'bg-red-700/50' : 'bg-[#f4f4f4]'}`}>
        ...
        <p className={`text-xs leading-relaxed font-medium mb-4 transition-colors duration-700 ${isActive ? 'text-red-100' : 'text-gray-500'}`}>{text}</p>
        <div className="flex flex-wrap gap-1 mt-auto">
          {tags.map(tag => (
            <span key={tag} className={`px-2 py-0.5 rounded-full text-[10px] font-bold transition-colors duration-700 ${isActive ? 'bg-white/20 text-white' : 'bg-gray-200 text-gray-600'}`}>{tag}</span>
          ))}
        </div>
    ...
    >
      <div className="max-w-6xl mx-auto relative md:h-[1600px]">
        ...
        <svg className="hidden md:block absolute top-0 left-0 w-full h-[1600px] pointer-events-none z-0" viewBox="0 0 1000 1600" preserveAspectRatio="none">
          <path d="M 650,200 C 400,350 200,500 300,750 C 400,1000 750,950 700,1150 C 650,1350 400,1400 300,1450" fill="none" stroke="#cbd5e1" strokeWidth="2" strokeDasharray="8 10" />
          <mask id="path-mask">
            <motion.path d="M 650,200 C 400,350 200,500 300,750 C 400,1000 750,950 700,1150 C 650,1350 400,1400 300,1450" fill="none" stroke="white" strokeWidth="20" style={{ pathLength }} />
          </mask>
          <path d="M 650,200 C 400,350 200,500 300,750 C 400,1000 750,950 700,1150 C 650,1350 400,1400 300,1450" fill="none" stroke="black" strokeWidth="2" strokeDasharray="8 10" mask="url(#path-mask)" className="drop-shadow-sm" />
        </svg>
        ...
        <div className="flex flex-col gap-8 md:gap-12 items-center md:block relative z-10 w-full pt-4 md:pt-0 pb-12 md:pb-0">
          ...
          <div data-aos="fade-in" data-aos-delay="600" className="hidden md:block absolute top-[1500px] left-[55%] font-['Caveat',cursive] text-3xl text-gray-600 rotate-6">
            More coming soon!
          </div>
        </div>
      </div>
